...
struct UbioBackstoreHost : public UBCCHostIf, public UBCCOutboundIf {
    ...
    static bool sameDataRequest(const CoherenceMessage &a,
        ...
        return a.b.writebackReq.hasData == b.b.writebackReq.hasData &&
            a.b.writebackReq.parentInvalidateReqId == b.b.writebackReq.parentInvalidateReqId &&
            a.b.writebackReq.parentInvalidateEpoch == b.b.writebackReq.parentInvalidateEpoch &&
    ...
    bool scheduleWritebackResponse(const CoherenceMessage &request,
                                   bool keepAsClean) {
        if (request.h.homeLinePa == 0x14030000 && framework::detail::LogDebugEnabled()) {
            framework::LogDebug("UBIO", "[WB-DIAG] stage=HOME_WRITE_REQ tick={} home={} socket={} pa=0x{:x} src={} sourceSocket={} requester={} epoch={} req={} kind={} disposition={} hasData={} mask=0x{:x} parent={} parentEpoch={} keepClean={}",
                tickRef, nodeId, socketId, request.h.homeLinePa, request.h.srcNode,
                request.h.srcSocket, request.h.requesterNode, request.h.epoch,
                request.h.reqId, static_cast<int>(request.b.writebackReq.kind),
                static_cast<int>(request.b.writebackReq.disposition),
                request.b.writebackReq.hasData, request.b.writebackReq.byteMask,
                request.b.writebackReq.parentInvalidateReqId,
                request.b.writebackReq.parentInvalidateEpoch, keepAsClean);
            if (request.b.writebackReq.hasData) {
                for (int i = 0; i < 64; i += 8) {
                    uint64_t word;
                    std::memcpy(&word, request.b.writebackReq.data + i, 8);
                    framework::LogDebug("UBIO", "[WB-DIAG] stage=HOME_PAYLOAD tick={} pa=0x{:x} src={} socket={} req={} offset={} word=0x{:x}",
                        tickRef, request.h.homeLinePa, request.h.srcNode,
                        request.h.srcSocket, request.h.reqId, i, word);
                }
            }
        }
        const DataTxnKey key = dataTxnKey(request);
        ...
        if (!ubcc.reserveWritebackPersistence(
                ...
                request.b.writebackReq.parentInvalidateReqId,
                request.b.writebackReq.parentInvalidateEpoch,
                request.b.writebackReq.parentInvalidateReqId ? request.h.srcNode : -1)) {
            if (request.h.homeLinePa == 0x14030000)
                framework::LogDebug("UBIO", "[WB-DIAG] stage=HOME_RESERVE_REJECT tick={} req={}", tickRef, request.h.reqId);
            pendingDataTxns.erase(key);
            pendingDataRequests.erase(key);
            return false;
        }
        ...
        if (request.h.homeLinePa == 0x14030000)
            framework::LogDebug("UBIO", "[WB-DIAG] stage=HOME_RESERVED tick={} req={} ownerWriteback={}", tickRef, request.h.reqId, ownerWriteback);
        const bool queued = dsmData.writeDataMaskedAsync(
            ...
            if (success && ownerWriteback && !keepAsClean &&
                ...
                if (request.h.homeLinePa == 0x14030000)
                    framework::LogDebug("UBIO", "[WB-DIAG] stage=HOME_RECALL_MERGED tick={} req={}", tickRef, request.h.reqId);
            } else {
                  ...
                  CoherenceMessage &response, bool &hasResponse)
{
    ...
    switch (msg.h.type) {
      ...
      case CoherenceMessageType::WritebackReq: {
        ...
        const bool validParent = msg.b.writebackReq.parentInvalidateReqId == 0
            ? msg.b.writebackReq.parentInvalidateEpoch == 0
            : (kind == UBWritebackKind::InternalPublication &&
               msg.b.writebackReq.byteMask == ~uint64_t{0} &&
               msg.h.homeNode == nid && msg.h.homeSocket == sid);
        ...
        if (!validKind || !validDisposition || !validPayload || !exactStoreOwner ||
            !validInternalIdentity || !validParent ||
            !plausibleOwner) {
            response.h.type = CoherenceMessageType::WritebackResp;
            response.h.srcNode = nid; response.h.srcSocket = sid;
            response.h.dstNode = msg.h.srcNode; response.h.dstSocket = msg.h.srcSocket;
