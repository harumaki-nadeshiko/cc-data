...
case "$MODE" in
    ...
    loss)
        ...
        ;;
    all)
        TC_LIST="117 118 119 148 149 150 151 152 153 154 155 156 157 158 159"
        RUN_NAME="fault_all_${RUN_STAMP}"
        ;;
    *)
        echo "usage: $0 {smoke|qualification|level2|loss|all}" >&2
        exit 2
        ;;
esac
...
docker run --rm --name "ubcc-${RUN_NAME}" --network none \
    ...
    env E2E_RUN_ID="$RUN_NAME" LOG_BASE="logs/$RUN_NAME" \
        TIMEOUT_SEC=1200 E2E_STALL_TIMEOUT_SEC=600 \
