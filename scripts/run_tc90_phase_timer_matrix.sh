#!/bin/bash
# Remeasure all TC90+ workloads that gained phase-level guest timers.
set -euo pipefail
...
# Newly instrumented topology-specific workloads.
run_profiles --8n1s 90 91 92 93 94 || exit 1
run_profiles --8n2s 95 96 97 98 99 100 101 || exit 1
# Newly instrumented 3-node workloads.
run_profiles --1s 102 110 111 112 113 114 115 116 117 118 119 120 121 122 123 124 || exit 1
...
# HA workloads received standardized phase markers via json_sample().
...
printf 'Phase-timer matrix complete: %s\n' "$LOG_ROOT"
