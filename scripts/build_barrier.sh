#!/bin/bash
# Build barrier_manager against the selected opaque framework backend.
...
g++ "${CXXFLAGS[@]}" "$ROOT/modules/barrier/barrier_main.cc" "$FW_LIB" "${LDFLAGS[@]}" "${BACKEND_LDFLAGS[@]}" -o "$OUT/barrier_manager"
