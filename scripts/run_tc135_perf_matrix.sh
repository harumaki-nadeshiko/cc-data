#!/bin/bash
# Run TC135-TC140 across naive, spill-noopt, and spill-opt profiles.
set -uo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
RUN_TAG="${RUN_TAG:-tc135_perf_matrix_$(date +%Y%m%d_%H%M%S)}"
...
PY
    then
        printf 'PASS\t%s\t%s\t--1s\tPASS\t%s\n' "$tc" "$profile" "$log_dir" >>"$MATRIX"
        return 0
    fi

    printf 'FAIL\t%s\t%s\t--1s\tFAIL\t%s\n' "$tc" "$profile" "$log_dir" >>"$MATRIX"
    return 1
}

failures=0
for tc in ${TC_LIST:-135 136 137 138 139 140}; do
    for profile in ${PROFILE_LIST:-naive spill-noopt spill-opt}; do
        run_case "$tc" "$profile" || failures=$((failures + 1))
    done
done

python3 "$ROOT_DIR/scripts/summarize_tc135_perf_matrix.py" "$LOG_ROOT" \
    >"$LOG_ROOT/summary.json"
printf 'Matrix complete: %s (%d failed cases)\n' "$LOG_ROOT" "$failures"
exit "$failures"
