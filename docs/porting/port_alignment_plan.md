...
## 2.1.1 去发送句柄 TxHandle

### 现状考察

当前发送采用**显式句柄**模型（`framework/Port.hh:47-63`、`framework/Port.cc:21-37`）：

```cpp
class TxHandle {
    MemMessage* buffer();  // 拿缓冲区填字段
    bool send();           // 提交 → 内部 doSend() + releaseSendSlot()
    void cancel();         // 放弃 → releaseSendSlot()
    bool valid() const;
};
TxHandle* Port::allocateSendBuffer(uint64_t ts);  // 返回句柄或 nullptr（槽忙）
```

内部机制：
- `Port` 有单个发送缓冲 `_sendBuf` + 占用标志 `_sendBufInUse` + 复用句柄对象 `_txHandle`。
- `allocateSendBuffer`：若 `_sendBufInUse` 为真则返回 `nullptr`（单发送槽保护）；否则
  清空 `_sendBuf`、打戳、置 `_sendBufInUse=true`、返回 `&_txHandle`。
- `TxHandle::send()` → `Port::doSend()`（真正 zmq send）→ `releaseSendSlot()`（清标志）。
- `TxHandle::cancel()` → `releaseSendSlot()`（不发送，仅清标志）。

**8 个业务调用点**（全部为 `X = port->allocateSendBuffer(t); ...; X->send()` 模式）：
...
> 注意调用点 #3 和 #7 有**失败回滚**逻辑：#3 在 `setPayload` 失败时直接 `return false`
> （当前**未** cancel，靠下次 allocate 覆盖——已是隐患）；#7 显式 `h->cancel()`。

### 目标态（对齐参考）

去掉 `TxHandle`，改为**缓冲区直接暴露 + 显式 send/abort**：

```cpp
// 新接口（保持"缓冲区不由句柄管理"这一核心；返回类型用 MemMessage*）
MemMessage* Port::allocateSendBuffer(uint64_t ts);  // 返回内部 _sendBuf 指针（或 nullptr）
bool        Port::send();                            // 提交当前缓冲区（原 doSend + 清占用）
void        Port::abortSend();                       // 放弃当前缓冲区（原 cancel，清占用）
```

- 调用方：`MemMessage* buf = port->allocateSendBuffer(t); if(!buf) {...} 填字段; port->send();`
- `send()`/`abortSend()` 作用于 Port 当前持有的 `_sendBuf`，**不再有句柄对象**。
- **保留 `_sendBufInUse` 单发送槽保护**：`allocateSendBuffer` 在槽忙时返回 `nullptr`
  （与参考 `alloc_buf` 返回 0 一致，见 `all.cpp:42`）；`send()`/`abortSend()` 清标志。

> 设计说明：为什么保留 `send()` 是成员方法而非 `send(msg)` 传参？参考 `send(msg)`
> 传的 `msg` 就是 `alloc_buf` 返回的那个内部缓冲指针，语义等价于"提交当前缓冲"。
> 我们用**无参 `send()` 提交当前 `_sendBuf`** 更简单且等价，避免调用方传错指针。
> 若审核要求严格对齐 `send(MemMessage*)` 签名，见 §2.1.1 附录 A 的可选方案。

### 逐文件改法

#### 文件：`framework/Port.hh`

1. **删除整个 `TxHandle` 类**（约 47-63 行，含前置注释块）。
2. 在 `Port` public 区，把
   ```cpp
   ...
   改为
   ```cpp
   // Allocate the send buffer stamped at `timestamp`. Returns the buffer to
   // fill (valid until send()/abortSend()) or nullptr if a send is already
   // in progress (single send slot). The buffer is owned by the Port; the
   // caller does NOT free it.
   MemMessage* allocateSendBuffer(uint64_t timestamp);
   // Commit the buffer filled after allocateSendBuffer(). Returns false on
   // transport failure. Releases the send slot either way.
   bool send();
   // Abandon the buffer without sending. Releases the send slot.
   void abortSend();
   ```
3. 在 `private` 区：
   - 删除 `friend class TxHandle;`
   - 删除 `void releaseSendSlot();`（其逻辑并入 send/abortSend；或保留为私有 helper，
     二选一，见下）
   - 把 `bool doSend();` 保留为私有（`send()` 内部调用它）或将其内容并入 `send()`。
     **推荐**：保留 `doSend()` 私有不变，`send()` = `doSend() + 清占用`。
   - 删除成员 `TxHandle _txHandle{this};`（约 131 行）。
   - 保留 `MemMessage _sendBuf;` 与 `bool _sendBufInUse = false;`。

#### 文件：`framework/Port.cc`

1. **删除 `TxHandle` 三个方法定义**（`buffer()`/`send()`/`cancel()`，约 21-37 行）。
2. `Port::allocateSendBuffer`（约 159-170 行）：
   - 返回类型 `TxHandle*` → `MemMessage*`。
   - 函数体末尾把
     ```cpp
     _sendBufInUse = true;
     ...
     return &_sendBuf;
     ```
   - 其余（状态检查、`_sendBuf.clear()`、打戳）**不变**。
     > 状态检查 `if (_state != PortState::READY || _sendBufInUse)` 会在 2.1.3 里再改；
     > 本项**先保留** `_state` 判断，只去句柄。
3. 新增 `Port::send()`（可放在原 `doSend` 之后）：
   ```cpp
   bool Port::send() {
       bool ok = doSend();
       _sendBufInUse = false;
...
4. 新增 `Port::abortSend()`：
   ```cpp
   void Port::abortSend() { _sendBufInUse = false; }
   ```
5. `releaseSendSlot()`：删除该方法（其调用点在 TxHandle 里，已随之删除）。
   检查 `terminate()`（约 112-114 行）里对 `releaseSendSlot()` 的调用：
   ```cpp
   if (_txSock && _sendBufInUse) { releaseSendSlot(); }
   ```
   改为直接 `_sendBufInUse = false;`。
6. `doSend()` 本体**不变**（仍 memcpy `_sendBuf` → zmq send）。
7. `emitSync()`（约 258-272 行）改用新接口：
   ```cpp
   bool Port::emitSync(uint64_t curTick) {
       if (_lastSyncTs > 0 && curTick - _lastSyncTs < _linkLatency) return true;
       MemMessage* buf = allocateSendBuffer(curTick);
       if (!buf) return false;
...
所有 8 处（含 emitSync 已在上面处理，剩 7 处业务点）：
...
- 新：
  ...
  if (!buf) { /* 失败处理 */ }
  /* 填字段 */
  port->send();   // 或 port->abortSend();
  ```

逐点：

- **#1 `networksim_main.cc:135`**：
  `TxHandle* fh = ...allocateSendBuffer(_tick);` → `MemMessage* buf = it->second->allocateSendBuffer(_tick);`
  删除 `MemMessage* buf = fh->buffer();`（buf 已是返回值）；`fh->send();` → `it->second->send();`。
  > 注意此处 alloc 与 send 是**同一个 port 对象** `it->second`。
- **#2 `barrier_main.cc:87`**：`ports[ni]->allocateSendBuffer(tick)` 返回 `buf`；
  `rel = rh->buffer()` 去掉，直接用返回的 `buf`；`rh->send()` → `ports[ni]->send()`。
- **#3 `ubio_main.cc:245`（sendCoh）**：`h=allocateSendBuffer` → `buf=allocateSendBuffer`；
  删 `buf = h?h->buffer():nullptr`；失败判断 `if(!buf)` 保留；
  `setPayload` 失败分支**必须调用 `port->abortSend()` 再 return false**（修正现有隐患）；
  `h->send()` → `port->send()`。
- **#4 `ubio_main.cc:683`（barrier 转发）**：`fh=alloc` → `fwd=alloc`（即返回值即缓冲）；
  删 `fwd = fh->buffer()`；`fh->send()` → `netPort->send()`。
- **#5 `ubio_main.cc:699`（barrier release）**：同模式，`rh->send()` → `gem5Port->send()`。
- **#6 `UBAdapter.cc:122`（sendBarrierReached）**：`h=alloc; if(!h) return;` → `buf=alloc; if(!buf) return;`；
  删 `buf=h->buffer()`；`h->send()` → `_port->send()`。
- **#7 `UBAdapter.cc:165`（transportSend）**：`h=alloc; if(!h) return false;` → `buf=alloc; if(!buf) return false;`；
  删 `buf=h->buffer()`；`setPayload` 失败：`h->cancel()` → `_port->abortSend()`；
  `if(!h->send())` → `if(!_port->send())`。

#### 文件：`framework/tests/port_sync_smoke.cc`

审查该测试是否使用 `TxHandle`/`allocateSendBuffer`/`send`；若有，按上述模式改。
（执行前先 `rg -n "TxHandle|allocateSendBuffer|->send\(|->buffer\(|->cancel\(" framework/tests/port_sync_smoke.cc`。）

### 风险与注意

- **R1（send/abort 忘记清占用）**：任何提前 return 的失败路径若不调用 `send()`/`abortSend()`，
  `_sendBufInUse` 会永久为真，下次 `allocateSendBuffer` 恒返回 nullptr → 该 port 发送
  彻底卡死。**执行时逐一核对每个失败分支都调用了 `abortSend()`**（尤其 #3、#7）。
- **R2（返回值即缓冲，勿再 buffer()）**：所有 `x->buffer()` 调用必须删除，改用
  allocate 的返回值。漏删会编译报错（TxHandle 已删），属好检测的错误。
- **R3（同一 port 对象）**：`send()`/`abortSend()` 是成员方法，必须在**执行 allocate 的
  那个 port 对象**上调用（#1 是 `it->second`，#2 是 `ports[ni]`）。
...
### 附录 A（可选，若审核要求严格 `send(MemMessage*)` 签名）

把 `bool send();` 改为 `bool send(MemMessage* buf);`，实现里 `assert(buf == &_sendBuf)`
后走 doSend。调用方 `port->send(buf)`。语义等价，仅签名更贴参考。**默认不采用**
（无参 `send()` 更不易误用）。
...
gem5 子模块：
```
2.1.1 (gem5): drop TxHandle; use Port::allocateSendBuffer/send/abortSend

UBAdapter transportSend/sendBarrierReached now fill the buffer returned by
allocateSendBuffer directly and commit via _port->send()/abortSend(), matching
the reference alloc_buf/send model (no send handle).
```
根仓库：
```
2.1.1: remove TxHandle send-handle; buffer no longer owned by a handle

Port::allocateSendBuffer now returns MemMessage* (the internal send buffer);
Port::send() commits it, Port::abortSend() abandons it. Single-send-slot
protection (_sendBufInUse) is kept. Updated all senders (ubio/networksim/
barrier) and emitSync. Fixes a latent leak: sendCoh's setPayload-failure path
now calls abortSend() instead of leaving the slot occupied.
```
...
3. 删除对外声明 `bool isReady() const;`、`PortState state() const;`、
   `void failClosed(const char*);`（若存在）。
4. 保留 `void terminate();`、`void closeLocal();`（生命周期仍需要，但内部不再依赖 _state，
   见下）。
5. 新增一个**最小私有布尔**用于替代 `init` 防重复与 socket 有效性判断：
   ```cpp
   bool _open = false;   // sockets bound/connected & usable; set by init, cleared by closeLocal
   ```
   > 说明：C3 要求 Port 不维护"对端是否 terminate/可用"这类**协议级**状态；但"socket
   > 是否已成功 init/是否已 closeLocal"是**资源生命周期**事实，不是协议状态。保留一个
   > `_open` 布尔用于避免对已关闭 socket 收发（防崩溃），符合参考精神（参考的 Port 也
   > 不会对未 init 的 channel 收发）。**这是资源事实，不是 PEER_LOST 那种协议状态。**
   > 若审核认为连 `_open` 也应外置，见 §2.1.3 附录 B。
...
1. `init()`：
   ...
   - 失败路径仍走 `closeLocal()`。
2. `closeLocal()`：
   - `if(_state==CLOSED) return; _state=CLOSED;` → `if(!_open) return; _open=false;`
     然后照旧 reset sockets/ctx。
3. `terminate()`：
   - `if(_state!=READY){closeLocal();return;}` → `if(!_open){closeLocal();return;}`
   - 删除 `_state = TERMINATING;`（无此状态了）。
   - 中段 `if(_txSock && _sendBufInUse){...}` 保留（2.1.1 已改为直接清标志）。
   - 结尾 `closeLocal();` 保留。
4. `failClosed()`：**删除整个方法**（其唯一职责是置 PEER_LOST，现由应用层接管）。
5. `safeTs()`：
   - **删除** `if(PEER_LOST||CLOSED||TERMINATING) return UINT64_MAX;` 整段。
   - 剩下的 `min(receiveTimestamp(), lookahead)` 逻辑**保留不变**。
   - > 这段删除后，"对端 terminate 后不阻塞全局"的责任**完全**转移到应用层
     > （不再 poll 该 port + 不计入 min）。见下"应用层改法"。
6. `allocateSendBuffer()`：
   - `if(_state != READY || _sendBufInUse)` → `if(!_open || _sendBufInUse)`。
7. `doSend()`：
   - `if(_state==PEER_LOST) return false; if(_state!=READY) return false;` →
...
- **R5（safeTs 去掉 +∞ 后的全局冻结）★本项最高风险**：删掉 `safeTs` 的 UINT64_MAX
  ...
  **删 safeTs 特判的配套义务**是：应用层必须"terminate 后不再 poll 该 port + 不把它计入
  ...
  务必逐进程核对 done 机制已落地。
- **R6（_open 与 2.1.1 顺序）**：本项依赖 2.1.1 已完成（send/abortSend 已就位）。**必须
  先做 2.1.1 再做 2.1.3。**
...
- **不要自行扩展 Port 接口**。最终 Port 公开接口应收敛为：`init / terminate /
  allocateSendBuffer / send / abortSend / receiveTimestamp / safeTs / emitSync / recv /
  closeLocal` + 只读 getter（moduleId/portId/syncInterval/name）。**不得**新增
  isReady/state/failClosed 等。
