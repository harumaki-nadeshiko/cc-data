...
| `CONDITIONAL PASS` | 在明确方案、完成点和参数/权重不等式成立时满足门槛，条件必须随结论同时披露 |
...
目标 1/2 等实测指标的最终合同 PASS 应以 `E5` 为目标。目标 3 是参数化理论指标，其证据闭环由
方案和假设签署、场景向量、break-even、参数来源与独立复核构成；可选实测不能替代理论证明，
理论证明也不能替代功能正确性和生产规模 E2E qualification。
