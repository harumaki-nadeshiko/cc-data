...
> 文档整合说明（2026-06-19）：原 `formal_verification_plan.md` 已并入本文。
> 本文同时作为“工程验证计划 + 形式化验证计划”的单一入口，避免双文档漂移。

## 0. 与原 formal_verification_plan 的合并锚点

为确保形式化细节不丢失，以下主题在本文后续章节统一覆盖：

- 一致性不变量（single-writer/multiple-reader、owner/数据一致性）
- 死锁/活锁可达性与公平性假设
- 消息重排/重复/丢失故障模型
- 与 `tc45_protocol_primitives.md` 的原语对齐验证
- 模型检查 + 仿真 + 故障注入的闭环验收标准
