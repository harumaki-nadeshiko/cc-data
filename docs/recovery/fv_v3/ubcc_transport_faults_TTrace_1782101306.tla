---- MODULE ubcc_transport_faults_TTrace_1782101306 ----
EXTENDS ubcc_transport_faults, Sequences, TLCExt, Toolbox, Naturals, TLC

_expression ==
    LET ubcc_transport_faults_TEExpression == INSTANCE ubcc_transport_faults_TEExpression
    IN ubcc_transport_faults_TEExpression!expression
----

_trace ==
    LET ubcc_transport_faults_TETrace == INSTANCE ubcc_transport_faults_TETrace
    IN ubcc_transport_faults_TETrace!trace
----
...
_inv ==
    ~(
        ...
        ostReservedEpoch = (1)
        /\
        tombstone = ((<<0, 0>> :> FALSE @@ <<0, 1>> :> FALSE @@ <<0, 2>> :> FALSE @@ <<0, 3>> :> FALSE @@ <<0, 4>> :> FALSE @@ <<1, 0>> :> FALSE @@ <<1, 1>> :> FALSE @@ <<1, 2>> :> FALSE @@ <<1, 3>> :> FALSE @@ <<1, 4>> :> FALSE @@ <<2, 0>> :> FALSE @@ <<2, 1>> :> FALSE @@ <<2, 2>> :> FALSE @@ <<2, 3>> :> FALSE @@ <<2, 4>> :> FALSE @@ <<3, 0>> :> FALSE @@ <<3, 1>> :> FALSE @@ <<3, 2>> :> FALSE @@ <<3, 3>> :> FALSE @@ <<3, 4>> :> FALSE @@ <<4, 0>> :> FALSE @@ <<4, 1>> :> FALSE @@ <<4, 2>> :> FALSE @@ <<4, 3>> :> FALSE @@ <<4, 4>> :> FALSE))
        /\
        ostAccepted = (FALSE)
        /\
...
_init ==
    ...
    /\ transportRecord = _TETrace[1].transportRecord
...
_next ==
    /\ \E i,j \in DOMAIN _TETrace:
        ...
        /\ transportRecord  = _TETrace[i].transportRecord
        /\ transportRecord' = _TETrace[j].transportRecord
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("ubcc_transport_faults_TTrace_1782101306.json", _TETrace)

=============================================================================

 Note that you can extract this module `ubcc_transport_faults_TEExpression`
  to a dedicated file to reuse `expression` (the module in the 
  dedicated `ubcc_transport_faults_TEExpression.tla` file takes precedence 
  over the module `ubcc_transport_faults_TEExpression` below).

---- MODULE ubcc_transport_faults_TEExpression ----
EXTENDS ubcc_transport_faults, Sequences, TLCExt, Toolbox, Naturals, TLC

expression == 
    [
        \* To hide variables of the `ubcc_transport_faults` spec from the error trace,
...
\*
\*---- MODULE ubcc_transport_faults_TETrace ----
\*EXTENDS ubcc_transport_faults, IOUtils, TLC
\*
\*trace == IODeserialize("ubcc_transport_faults_TTrace_1782101306.bin", TRUE)
\*
\*=============================================================================
\*

---- MODULE ubcc_transport_faults_TETrace ----
EXTENDS ubcc_transport_faults, TLC

trace == 
    <<
    ([ostTargetMask |-> {},transportRecord |-> [kind |-> "INIT", prevState |-> "G_I", prevSharers |-> {}, prevOwner |-> -1, prevDirty |-> FALSE, prevEpoch |-> 0],ostReservedEpoch |-> 0,tombstone |-> (<<0, 0>> :> FALSE @@ <<0, 1>> :> FALSE @@ <<0, 2>> :> FALSE @@ <<0, 3>> :> FALSE @@ <<0, 4>> :> FALSE @@ <<1, 0>> :> FALSE @@ <<1, 1>> :> FALSE @@ <<1, 2>> :> FALSE @@ <<1, 3>> :> FALSE @@ <<1, 4>> :> FALSE @@ <<2, 0>> :> FALSE @@ <<2, 1>> :> FALSE @@ <<2, 2>> :> FALSE @@ <<2, 3>> :> FALSE @@ <<2, 4>> :> FALSE @@ <<3, 0>> :> FALSE @@ <<3, 1>> :> FALSE @@ <<3, 2>> :> FALSE @@ <<3, 3>> :> FALSE @@ <<3, 4>> :> FALSE @@ <<4, 0>> :> FALSE @@ <<4, 1>> :> FALSE @@ <<4, 2>> :> FALSE @@ <<4, 3>> :> FALSE @@ <<4, 4>> :> FALSE),ostAccepted |-> FALSE,ostStage |-> "CREATED",ostIntendedSharers |-> {},tick |-> 0,ostOpType |-> "NONE",committedSharers |-> {},ostIntendedState |-> "G_I",ostAckMask |-> {},ostBaseEpoch |-> 0,committedEpoch |-> 0,ostRecallDone |-> FALSE,committedState |-> "G_I",commitLog |-> <<>>,ostRequester |-> -1,messages |-> {},committedOwner |-> -1,committedDirty |-> FALSE,ostIntendedOwner |-> -1,ostInvalidateDone |-> FALSE,ostReqId |-> 0]),
    ([ostTargetMask |-> {},transportRecord |-> ,ostReservedEpoch |-> 1,tombstone |-> (<<0, 0>> :> FALSE @@ <<0, 1>> :> FALSE @@ <<0, 2>> :> FALSE @@ <<0, 3>> :> FALSE @@ <<0, 4>> :> FALSE @@ <<1, 0>> :> FALSE @@ <<1, 1>> :> FALSE @@ <<1, 2>> :> FALSE @@ <<1, 3>> :> FALSE @@ <<1, 4>> :> FALSE @@ <<2, 0>> :> FALSE @@ <<2, 1>> :> FALSE @@ <<2, 2>> :> FALSE @@ <<2, 3>> :> FALSE @@ <<2, 4>> :> FALSE @@ <<3, 0>> :> FALSE @@ <<3, 1>> :> FALSE @@ <<3, 2>> :> FALSE @@ <<3, 3>> :> FALSE @@ <<3, 4>> :> FALSE @@ <<4, 0>> :> FALSE @@ <<4, 1>> :> FALSE @@ <<4, 2>> :> FALSE @@ <<4, 3>> :> FALSE @@ <<4, 4>> :> FALSE),ostAccepted |-> FALSE,ostStage |-> "WAITING_CLEAR",ostIntendedSharers |-> {0},tick |-> 0,ostOpType |-> "GRANT_HANDSHAKE",committedSharers |-> {},ostIntendedState |-> "G_S",ostAckMask |-> {},ostBaseEpoch |-> 0,committedEpoch |-> 0,ostRecallDone |-> FALSE,committedState |-> "G_I",commitLog |-> <<>>,ostRequester |-> 0,messages |-> ,committedOwner |-> -1,committedDirty |-> FALSE,ostIntendedOwner |-> -1,ostInvalidateDone |-> FALSE,ostReqId |-> 0])
    >>
----
...
---- CONFIG ubcc_transport_faults_TTrace_1782101306 ----
CONSTANTS
    Nodes = { 0 , 1 , 2 }
    MaxEpoch = 4
    TombstoneWindow = 10
...
ALIAS
    _expression
=============================================================================
\* Generated on Mon Jun 22 12:08:27 CST 2026