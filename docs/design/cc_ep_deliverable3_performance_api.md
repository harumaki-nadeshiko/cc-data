...
| 项目 | 结论 | 适用范围 |
|---|---|---|
| 指标 1 | 容量 1.515×；附加时延 10.534578 ns / 21.069156 cycles | 当前两个合格证据集按各自冻结职责组合计分 |
...
1. 容量证据取自 legacy 72-run 指标 1/2 profile 矩阵中的三次 naive 和三次 spill-noopt；
...
验证环境曾使用 NetworkSim 作为临时模拟传输承载跨进程消息；它不属于项目架构、交付组件
或公开接口。
