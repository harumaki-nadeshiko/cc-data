...
本次可靠性结论面向可恢复传输故障。故障注入不改变节点和 Home 的持久存活假设。

| 故障类别 | 模型 | 预期恢复机制 | 本次证据边界 |
|---|---|---|---|
...
| 永久节点或 Home 故障 | 节点不再恢复服务 | 需要冗余 Home、数据副本与在线重配置 | 不在本次范围 |
...
图 5-1　Q1-Q5 故障资格结果。五组共 52 项物理运行，最终结果为 52/52 通过。
...
HA-VI 证据用于回答：在相同 workload、完成边界和冻结参数下，UBCC 的目录定位与权限路径
相对窄化 VI 参考模型有何结构差异。比较范围固定为 2N1S、O3、单向完成语义，以及 100%
和 150% L3 压力；它是理论与可执行参考比较，不是物理芯片测量。

### 6.2 假设与公平性边界

| 维度 | 客户确认 / 冻结参考假设 | 未建模硬件或禁止外推项 |
|---|---|---|
| 系统与处理器 | 2N1S、O3；UBCC 与 HA-VI 使用相同节点和处理器条件 | 客户物理芯片的核、缓存和 Home 微体系结构 |
| workload 与输入 | 相同 testcase、输入、操作配比和 L3 压力点 | 未冻结 workload、流量分布和更大规模压力 |
| 完成与计量 | 相同根操作完成边界；单向完成语义 | 物理链路握手、未纳入主值的内部服务阶段 |
| 对照模型 | 冻结 HA-VI 可执行参考模型及其目录、状态和时延参数 | 客户 HA 的私有优化、缓冲、队列、预测和一致性扩展 |
| 通信结构 | 两方案使用相同可执行比较环境 | 端口级 Switch 排队、仲裁、拥塞和链路误码 |
| 结果解释 | 按冻结主值形成核心场景组和代表场景组的等权聚合 | 将参考模型结果宣称为客户物理实现结果 |

这些条件保证配对比较在已确认参考范围内使用共同输入和共同完成边界。聚合优势表示冻结
场景组的平均主值占优，不表示每一个 testcase、内部阶段或子操作都必须占优。
...
理论比较只解释结构差异，不单独构成性能胜负结论。性能裁决来自相同输入下的冻结配对运行。
...
在该冻结范围内，两压力点的核心场景组和代表场景组均满足 UBCC 平均时延低于 HA-VI。
该结果与理论分析相互印证，但不外推到未建模 HA 机制或客户物理实现。
...
### 7.2 16 节点能力与 Switch 边界

16N1S Level-A 验证覆盖 16 个协议节点、跨节点共享者集合和正式性能工作负载。Q5 进一步
覆盖该配置下的请求丢失、Ack 丢失和 Ack 延迟。这里的 Level-A 结论是节点规模与协议
端点能力结论；Switch 内部微体系结构、端口、排队、仲裁和拥塞模型未作为新的独立实验对象，因而
不将该结果表述为端口级 Switch 实现验证。
...
物理芯片 HA 微体系结构和端口级 Switch 排队、仲裁与拥塞不属于本次验证结论。
