...
| HA 比较 | 冻结 HA-VI 参考范围内聚合结果支持 UBCC 优势 | 2N1S、O3、单向完成语义和固定 256 KiB L3 配置 |
...
| 本次范围外 | 永久 Home 失效恢复、目标物理 HA 和端口级 Switch 微体系结构 | 明确交付边界 | 已验证或已实现能力 |
...
L3 和 100% L3 压力；它是理论与可执行参考比较，不是物理芯片测量。

### 6.2 假设与公平性边界

| 维度 | 冻结参考条件 | 未建模硬件或禁止外推项 |
|---|---|---|
| 系统与处理器 | 2N1S、O3；UBCC 与 HA-VI 使用相同节点和处理器条件 | 目标物理芯片的核、缓存和 Home 微体系结构 |
| workload 与输入 | 相同 testcase、输入、操作配比和 100% L3 压力 | 未冻结 workload、流量分布和其他 L3 配置 |
| 完成与计量 | 相同根操作完成边界；单向完成语义 | 物理链路握手、未纳入主值的内部服务阶段 |
| 对照模型 | 冻结 HA-VI 可执行参考模型及其目录、状态和时延参数 | 未建模 HA 的优化、缓冲、队列、预测和一致性扩展 |
| 通信结构 | 两方案使用相同可执行比较环境 | 端口级 Switch 排队、仲裁、拥塞和链路误码 |
| 结果解释 | 按冻结主值形成核心场景组和代表场景组的等权聚合 | 将参考模型结果外推为目标物理实现结果 |

这些条件保证配对比较在冻结参考范围内使用共同输入和共同完成边界。聚合优势表示冻结
...
该结果与理论分析相互印证，但不外推到未建模 HA 机制或目标物理实现。
...
该配置下的请求丢失、Ack 丢失和 Ack 延迟。这里的 Level-A 结论是节点规模与协议端点能力结论；
Switch 内部微体系结构、端口、排队、仲裁和拥塞模型未作为新的独立实验对象，因而不将该结果
表述为端口级 Switch 实现验证。
