...
```bash
cd tools/wheels/aarch64-cp38 && sha256sum -c SHA256SUMS && cd ../../..
```
...
python3 -m pip install \
  --no-index \
  --find-links=tools/wheels/aarch64-cp38 \
  -r tools/wheels/aarch64-cp38/requirements.txt
```
...
```bash
python3 -c 'import matplotlib, numpy, PIL; print(matplotlib.__version__, numpy.__version__, PIL.__version__)'
```
