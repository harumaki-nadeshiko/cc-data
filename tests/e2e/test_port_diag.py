"""Port connection diagnostic - verify CPU <-> RubySequencer port binding.
"""
import sys, os
...
if __name__ == "__m5_main__":
    ...
    if not os.path.exists(binary):
        src = os.path.join(SCRIPT_DIR, "workloads", "e2e_tc_minimal.c")
        subprocess.run(["aarch64-linux-gnu-gcc", "-static", "-O0", "-g",
    ...
    for i, seq in enumerate(cpu_sequencers):
        print(f"[DIAG] Connecting CPU{i} icache_port -> inst_seq.in_ports", flush=True)
        print(f"[DIAG]   cpu._cached_ports = {cpus[i]._cached_ports}", flush=True)
        seq.connectCpuPorts(cpus[i])

    # Check port connection counts BEFORE instantiate
    for i, seq in enumerate(cpu_sequencers):
        iseq = seq.inst_seq
        dseq = seq.data_seq
        try:
            ic = iseq.get_port_connection_count("in_ports")
            dc = dseq.get_port_connection_count("in_ports")
            print(f"[DIAG] CPU{i} seq: inst_seq.in_ports conn_count={ic}, "
                  f"data_seq.in_ports conn_count={dc}", flush=True)
        except Exception as e:
            print(f"[DIAG] CPU{i} seq: cannot read conn_count: {e}", flush=True)
    ...
    print(f"[DIAG] Pre-mapped {_total_pages} pages", flush=True)
    ...
    # Dump port connection status just before instantiate
    print(f"[DIAG] === Pre-instantiate port check ===", flush=True)
    try:
        # Check if system.mem_ranges covers the expected addresses
        print(f"[DIAG] mem_ranges count: {len(system.mem_ranges)}", flush=True)
        for r in system.mem_ranges[:5]:
            print(f"[DIAG]   {r}", flush=True)
    except Exception as e:
        print(f"[DIAG] Cannot read mem_ranges: {e}", flush=True)

    m5.instantiate()
    
    # Post-instantiate check
    print(f"[DIAG] === Post-instantiate check ===", flush=True)
    for cpu in cpus:
        try:
            print(f"[DIAG] CPU{cpu.cpu_id}: icache_port.isConnected()={cpu.icache_port.isConnected()}",
                  flush=True)
        except Exception as e:
            print(f"[DIAG] CPU{cpu.cpu_id}: {e}", flush=True)
