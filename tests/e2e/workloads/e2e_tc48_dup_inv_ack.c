 ...
 *   Fault rule duplicates the InvalidateAck from Node2.
 *   Idempotent ack handling should absorb the duplicate.
 *   Final read must see the upgraded value (0x48BB0022).
 *
 * Fault config (applied in test_e2e.py):
 *   Rule: match InvalidateAck, src=2, dst=0 → duplicate (once)
 */
#include "dsm_access.h"
#include "e2e_common.h"

#define HOME_NODE 0
#define MAIN_OFF   0x4800
...
int main(int argc, char **argv)
{
    ...
    emit_e2e_meta(node_id, "TC48");
    int fail = 0;
    uint32_t init_val = 0x48AA0011u;
    uint32_t final_val = 0x48BB0022u;
    ...
    /* Phase 4: All nodes read — must see final_val */
    if (node_id == 0) {
        uint32_t got = dsm_load(HOME_NODE, MAIN_OFF);
