...
int main(int argc,char **argv) {
 int n=0,c=0;if(argc>=2)n=parse_int(argv[1]);if(argc>=3)c=parse_int(argv[2]);if(c%4){_exit_program(0);return 0;}emit_e2e_meta(n,"TC132");emit_timer_selftest(n);
 ...
 if(n==2){uint64_t t0=read_cntvct_el0();for(int i=0;i<ACTIVE;i++){uint32_t v=dsm_load(0,active_off(i));if(i%TC132_READ_STRIDE==0)emit_read_val(2,0,VALUE|(uint32_t)i,v,v==(VALUE|(uint32_t)i));}emit_guest_timer(2,"post_pressure_checkpoint_recover",ACTIVE,read_cntvct_el0()-t0);emit_phase_done(2,"checkpoint_recover");} sync_wait(7);_exit_program(0);return 0;
}
