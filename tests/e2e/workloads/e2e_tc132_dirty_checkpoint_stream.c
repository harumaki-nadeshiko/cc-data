...
int main(int argc,char **argv) {
 ...
 if(n==2){for(int i=0;i<ACTIVE;i++){uint32_t v=dsm_load(0,active_off(i));if(i%512==0)emit_read_val(2,0,VALUE|(uint32_t)i,v,v==(VALUE|(uint32_t)i));}emit_phase_done(2,"checkpoint_recover");} sync_wait(7);_exit_program(0);return 0;
}
