/* TC98: 8-node dual-socket same-home contention.
 * Each of 16 socket-plane primaries writes to a PER-PLANE private slot on
 * home node 0, socket 0 — not to the same cache line.  This avoids extreme
 * directory serialization (O(n^2) on a single PA) while still exercising
 * the home UBCC's outstanding / replayArmed pipeline with 16 concurrent
 * requestors.  After N rounds, barrier, node 0 reads all done markers.
 */
...
{
    /* Each socket-plane gets its own cache-line-aligned slot on home 0,0.
     * Offsets: 0x7800, 0x7840, 0x7880, ... — one cache line apart. */
...
int main(int argc, char **argv)
{
    ...
    /* Write final done marker to own slot */
    ...
    *my_slot(node_id, socket_id) = done_val;
    ...
    /* Node 0 socket 0 reads all 16 done markers */
