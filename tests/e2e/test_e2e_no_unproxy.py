"""E2E test WITHOUT _early_unproxy_all to check if port binding works.
Based on test_e2e.py TC1, but skips the proxy resolution workaround.
"""
...
if __name__ == "__m5_main__":
    ...
    # NOTE: SKIP _early_unproxy_all — test if the issue is proxy-related
    ...
    print(f"[NO-UNPROXY] Creating Ruby system...", flush=True)
    ...
    print("=" * 60, flush=True)
    print("NO-UNPROXY CPU TEST", flush=True)
    print("=" * 60, flush=True)
    ...
    if os.path.exists(simout_path):
        with open(simout_path) as f:
            for line in f:
                if "SENTINEL" in line:
                    print(f"  SENTINEL: {line.strip()}", flush=True)
